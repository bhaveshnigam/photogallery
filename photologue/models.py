...
class ImageModel(models.Model):
    image_path = models.CharField(max_length=IMAGE_FIELD_MAX_LENGTH)
    ...
    def _get_image(self):
        ...
        _image_field = models.ImageField(storage=_storage)
        return ImageFieldFile(self, _image_field, self.image_path)

    def _set_image(self, instance):
        self.image_path = str(instance)
...
class Photo(ImageModel):
    ...
    def raw_photo_url(self):
        ...
        if raw_results:
            ...
            if path:
                return 'http://%s/media%s' % (Site.objects.get_current().domain, path)
            return ''
        return ''
